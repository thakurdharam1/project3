...
function validateAlphaNum(inputElement, errorElementId, maxLength) {
    ...
    if (currentValue.length > 0 && !/^[a-zA-Z]/.test(currentValue)) {
        errorMessage.textContent = 'Input must start with a letter.';
        // Remove leading non-letters if they exist
        currentValue = currentValue.replace(/^[^a-zA-Z]+/, '');
    }
...
/*function validateAlphaNumInput(inputElement, errorElementId, maxLength) {
    const errorMessage2 = document.getElementById(errorElementId);
    const currentValue2 = inputElement.value;

    if (/^[a-zA-Z0-9]/.test(currentValue2) && currentValue2.length <= maxLength) {
        errorMessage.textContent = '';
    }
}*/
