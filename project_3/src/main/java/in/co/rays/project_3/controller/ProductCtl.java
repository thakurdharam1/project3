...
public class ProductCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("productName"))) {
			...
			request.setAttribute("productName", "Only letters are allowed");
			pass = false;
		}else if (DataValidator.isTooLong(request.getParameter("productName"),20)) {
			request.setAttribute("productName", "Only 20 Characters are allowed ");
			pass = false;
		}
		if (DataValidator.isNull(request.getParameter("productAmmount"))) {
			...
			request.setAttribute("productAmmount", "Only numbers are allowed ");
			pass = false;
		}else if (DataValidator.isTooLong(request.getParameter("productAmmount"),7)) {
			request.setAttribute("productAmmount", "Only 7 digits are allowed ");
			pass = false;
		}
	...
	protected BaseDTO populateDTO(HttpServletRequest request) {
		...
		dto.setProductAmmount(DataUtility.getString(request.getParameter("productAmmount")));
		dto.setPurchaseDate(DataUtility.getDate(request.getParameter("purchaseDate")));
		dto.setProductCategory(DataUtility.getString(request.getParameter("productCategory")));
